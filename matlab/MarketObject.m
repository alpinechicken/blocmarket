classdef MarketObject < handle
    ...
    methods (Access = public) % Construct and create users/market/trade
        ...
        % Public access collateral check: 
        
        function [colChk, netCollateral, colChkAll] = checkCollateral_public(this, newTrade)
            ...
            [colChk, netCollateral, colChkAll] = this.checkCollateral(newTrade);
    ...
    methods (Access = private) % Check collateral and match trades
        ...
        function [colChk, netCollateral, colChkAll] = checkCollateral(this, newTrade)
            ...
            %
            % Notr that this can br used to calculate collateral for any
            % subset of trades (including e.g. for a single user)
        ...
        function this = matchTrades(this)
            ...
            for iMarket = 1 : height(this.marketTable)
                ...
                allClear = false;
                while ~allClear
                    [colChk] = this.checkCollateral(table());
                    if all(colChk)
                        allClear = true;
                    else
                        ...
                        for iTrader = 1: length(tIds)
                            ...
                            this = this.removeMarginalTrade(tIds(iTrader));
                        end % iTrader
