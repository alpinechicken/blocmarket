classdef MarketObject < handle
    ...
    properties (SetAccess=private)
        ...
        % Market state:
        outcomeCombinations = table([], [], [], [], [], 'VariableNames',...
            {'outcomeId','marketRootId', 'marketBranchId', 'marketMin', 'marketMax'});   
        
        % (table) Possible combinations of root market outcomes
        marketBounds = table([], [], [], [], 'VariableNames',...
            {'marketRootId', 'marketBranchId', 'marketMin', 'marketMax'}); 
        marketOutcomes % (matrix) Market outcome matrix across states (all markets)
    ...
    methods (Access = public) % Construct and create users/market/trade
        ...
        function this = createMarket(this, newMarket)
            ...
            %
            % e.g. 
            %   newMarket = mc.marketMaker(mo.getPreviousMarket, 1, 2, 0.1, 0.7, 1)
            %   mo = mo.createMarket(newMarket)           
            %
    ...
    methods (Access = public) % Get previous market/trade  
        ...
        function previousTrade = getPreviousTrade(this)
            ...
            % 
            % Out:
            % previousTrade (table row) - most recent valid trade 
    ...
    methods (Access = private) % Construct and update state for market/trades
        ...
        function this = updateTradeState(this)
            ...
            %
            % Out:
            % this.tradeState (table with tradeRootId/tradeBranchId/isOpen/isOffset/isMatched)
        ...
        function this = updateOutcomeCombinations(this)
            ...
            % (marketTable rows for root markets with minimum = maximum)
    ...
    methods (Access = public)
        ...
        function [colChk, netCollateral, colChkAll] = checkCollateral(this, newTrade)
            ...
            %
            % In: 
            % newTrade - (table row or struct) for this.orderBook
            %
            % Out:
            % colChk (logical 1*1) - collateral check for newTrade  
            ...
            for iTrade = 1 : numTrades
                ...
                IM(:, iTrade) = this.constructUnitVector(numMarkets, marketInd)';
            end % iTrade
            ...
            for iTrade = 1 : numTrades
                IQ(iTrade, :) = this.constructUnitVector(numTraders, allTrades.traderId(iTrade));
            end % iTrade
    ...
    methods (Access = private) % Match trades
        ...
        function this = matchTrades(this)
            ...
            %
            % Out: 
            % this.orderBook - matched trades
            
            % TODO: don't need to match across all markets every time
    ...
    methods (Access = private) % Mutate trades (write new/matched/remove trade)
        ...
        function this = writeMatchedTrade(this, targetTrade)
            ...
            %
            % In: 
            % targetTrade - (this.orderBook row)
            %
            % Out:
        ...
        function this = writeRemoveTrade(this, traderId)
            ...
            % In: 
            % traderId (integer) trader id to remove trade from
            %
            % Out:
            % this.orderBook  (new order)
            % this.tradeState (updated trade state)           
    ...
    methods (Access = private) % Find offset/matched/removed
        ...
        function offsetTrade = findOffsetTrade(this, targetTrade)
            ...
            %
            % In: 
            % targetTrade (this.orderBook row) - target trade
            %
            % Out:
            % offsetTrade (this.orderBook row) - offset trade             
        ...
        function removeTrade = findRemoveTrade(this, traderId)
            ...
            %
            % In: 
            % targetTrade (this.orderBook row) - trade asking for removal 
            %
            % Out:
            % removeTrade (this.orderBook row) - trade identified for removal 
    ...
    methods (Access = private) % Construct ouput combinations
        ...
        function marketOutcomes = constructOutcomeCombinations(this, marketTable)
            ...
            %
            % In: 
            % marketTable (marketTable table) - markets for combinations
            %
            % Out:
            % marketOutcomes (marketTable table) - extreme outcomes
            ...
            marketCombinations = this.constructCartesianProduct(exOutcome);
            ...
            for iOutcome = 1 : numCombinations
                ...
                for iMarket = 1 : numMarkets
                    mT{mT.marketRootId == marketIds(iMarket),...
                        ...
                        marketCombinations(iOutcome, iMarket)];
                    mT.outcomeId = repmat(iOutcome, height(mT),1);
                end % jOutcome
                marketOutcomes = vertcat(marketOutcomes, mT);
            end % iMarket
    ...
    methods (Access = private, Static) % Static functions (marketBounds and cartesian product)
        ...
        % constructCartesianProduct - constructCartesianProduct product
        % constructUnitVector - unit vector with one in some position and zeros
        %                  otherwise
        ...
        function marketBounds = constructMarketBounds(marketTable)
            ...
            % In:
            % marketTable (this.marketTable table) -  marketTable table same format as root  
            %
            % Out:
            % marketBounds ( table) -  {'marketRootId', 'marketBranch', 'marketMin' , 'marketMax'}
        ...
        function C = constructCartesianProduct(input)
            % Construct constructCartesianProduct product of e.g. {{[1 2]}, {[2 3]}, {[4,5]}}.
            ...
            C = unique(G , 'rows');
        end % constructCartesianProduct
        
        function u = constructUnitVector(L, x)
            % Make unit vector
            %
            % In: 
            % L (integer) - length of vector
            % x (integer) - position for '1'
            %
            % Out: 
            % u (1xL vector) - unit vector length L with 1 in x positions
            %
            % e.g. constructUnitVector(5,3) -> [0 0 1 0 0]
            I = eye(L);
            u = I(x, :);
        end% constructUnitVector
