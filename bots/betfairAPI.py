...
Get session key
"""
def getBetfairSessionToken(userName, appName, betfairAppKey, betfairPassword):
    ...
    content = 'username=' +userName+ ' &password=' + betfairPassword
...
def getMarketCatalogueForNextStarts(eventTypeID):
    # Get catalogue for next starts (just match odds). See matlab betfair api for more detail.
    if (eventTypeID is not None):
        print ('Calling listMarketCatalouge Operation to get MarketID and selectionId')
        now = datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%SZ')
        market_catalogue_req = '{"jsonrpc": "2.0", "method": "SportsAPING/v1.0/listMarketCatalogue", "params": {"filter":{"eventTypeIds":["' + eventTypeID + '"],"marketTypeCodes":["MATCH_ODDS"],'\
                                                                                                                                                             '"marketStartTime":{"from":"' + now + '"}},"sort":"FIRST_TO_START","maxResults":"100","marketProjection":["COMPETITION","EVENT","EVENT_TYPE","MARKET_DESCRIPTION","RUNNER_DESCRIPTION","RUNNER_METADATA","MARKET_START_TIME"]}, "id": 1}'
        """
print(market_catalogue_req)
"""
        market_catalogue_req = '{"jsonrpc": "2.0", "method": "SportsAPING/v1.0/listMarketCatalogue", "params": {"eventTypeIds": 1477, "marketBettingTypes": "ODDS", "marketTypeCodes": "MATCH_ODDS"},"maxResults": 20,"marketProjection": ["COMPETITION", "EVENT", "EVENT_TYPE", "MARKET_DESCRIPTION", "RUNNER_DESCRIPTION","RUNNER_METADATA", "MARKET_START_TIME"]}, "id": 1}'
        market_catalogue_response = callAping(market_catalogue_req)
        """
print(market_catalogue_response)
"""
...
def getMarketBookBestOffers(marketId):
    ...
    market_book_req = '{"jsonrpc": "2.0", "method": "SportsAPING/v1.0/listMarketBook", "params": {"marketIds":[tyy"' + marketId + '"],"priceProjection":{"priceData":["EX_BEST_OFFERS"]}}, "id": 1}'
    """
...
if ( args < 3):
    ...
    betfairPassword = input('Enter your password :')
    print ('Thanks for the input provided')
else:
    userName = sys.argv[1]
    betfairPassword = sys.argv[2]

betfairAppKey = 'iw8UsiHCP1GSs213'
betfairAppKey = 'zBBL3MKepCMp7HGd' # Live
appName = 'alpinechickenbetfair'

sessionToken = getBetfairSessionToken(userName, appName, betfairAppKey, betfairPassword)

headers = {'X-Application': betfairAppKey, 'X-Authentication': sessionToken, 'content-type': 'application/json'}

eventTypesResult = getEventTypes()
rugbyLeagueEventTypeID = getEventTypeIDForEventTypeName(eventTypesResult, 'Rugby League')

print ('Eventype Id for League is :' + str(rugbyLeagueEventTypeID))

marketCatalogueResult = getMarketCatalogueForNextStarts(rugbyLeagueEventTypeID)
