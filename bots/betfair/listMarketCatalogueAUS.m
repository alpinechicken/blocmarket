function [marketList] = listMarketCatalogueAUS(sessionKey,appKey,varargin)
...
url = ['https://api-au.betfair.com/exchange/betting/json-rpc/v1/']%'https://api.betfair.com/exchange/betting/json-rpc/v1'];
    ...
    for k= 1 : 2 : size(varargin,2)
        ...
        if isfield(bodyFields,varargin{k})
            ...
            if strcmp(varargin{k},'inPlayOnly')
                %Edit to solve bug with in play switch
                if varargin{k+1} == 1
                    varargin{k+1} = 'true';
                end
                if varargin{k+1} == 0
