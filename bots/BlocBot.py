...
class BlocBot(object):
    ...
    # Basic price making bot using external source data.
    ...
    def getQuote(self):
        ...
        if self.quoteSource == 'betfair':
            ...
            #  set_trace()
            bids = pd.DataFrame(orderBook[0]['runners'][self.betfairRunnerId]['ex']['availableToBack'])
            asks = pd.DataFrame(orderBook[0]['runners'][self.betfairRunnerId]['ex']['availableToLay'])
